...
class lang_reber:
    ...
    def get_sequence(self):
        ...
        seq = torch.from_numpy(np.asarray(seq_raw).astype(np.int64))
    ...
    def print_outputs(self,epoch,seq,state,hidden,target,output,gates=None):
        ...
        if self.embed:
            ...
            print('hidden activations, output probabilities [BTSXPVE] and memory units(input_gate, forget_gate,new value ,output_gate):')
            for k in range(len(state)-1):
                #edit here
                print(state[k+1],hidden_np[k,:],prob_out_np[k,:], gates_np[k, :])
        else:
            print('state = ',*state,sep='')
            print('symbol= '+''.join(symbol))
            print('label = ',*(seq.squeeze().tolist()),sep='')
