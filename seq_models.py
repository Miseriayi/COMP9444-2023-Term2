...
class LSTM_model(nn.Module):
    ...
    def forward(self, x, init_states=None):
        ...
        for t in range(seq_size):
            ...
            #edit here to return the gates values and print
            #my code
            r_t = torch.cat((i_t,f_t,g_t,o_t),dim=1)
            ##
            c_t = f_t * c_t + i_t * g_t
            h_t = o_t * torch.tanh(c_t)
            hidden_seq.append(h_t.unsqueeze(0))
            gates_value.append(r_t.unsqueeze(0))
        hidden_seq = torch.cat(hidden_seq, dim=0)
        gates_value = torch.cat(gates_value,dim=0)
        # reshape from (sequence, batch, feature)
        #           to (batch, sequence, feature)
        hidden_seq = hidden_seq.transpose(0,1).contiguous()
        output = hidden_seq @ self.V + self.out_bias
        return hidden_seq, output, gates_value
